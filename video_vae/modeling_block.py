...
from .modeling_resnet import (ResnetBlock2D, 
                                Downsample2D, 
...
def get_down_block(
    ...
    if down_block_type == "DownEncoderBlock2D":
        return DownEncoderBlock2D(num_layers=num_layers,
                                  in_channels=in_channels,
                                  out_channels=out_channels,
                                  dropout=dropout,
        ...
        return DownEncoderBlockCausal3D(num_layers=num_layers,
                                        in_channels=in_channels,
                                        out_channels=out_channels,
                                        dropout=dropout,
                                        resnet_eps=resnet_eps,
    ...
    raise ValueError(f"{down_block_type} does not support. please choose on this list [`DownEncoderBlock`, `DownEncoderBlockCausal3D`]")
    ...
    def __init__(self,
        ...
        if add_spatial_downsample:
            self.downsamplers = nn.ModuleList(
                [
                    Downsample2D(channels=out_channels,
                                 use_conv=True,
                                 out_channels=out_channels,
        ...
        if add_temporal_downsample:
            self.temporal_downsamplers = nn.ModuleList(
                [
                    TemporalDownsample2x(channels=out_channels,
                                         use_conv=True,
                                         out_channels=out_channels,
                                         padding=downsample_padding)
                ]
    ...
    def __init__(
        ...
        for _ in range(num_layers):
            if self.add_attention:
                ...
                ImportError("`Attention` does not import from `diffusers.models.attention_processor`")
...
class CausalUNetMidBlock2D(nn.Module):
    ...
    def forward(self,
        ...
        for attn, resnets in zip(self.attentions, self.resnets[1:]):
            if attn is not None:
                hidden_states = rearrange(hidden_states, 'b c t h w -> b t c h w')
                hidden_states = rearrange(hidden_states, 'b t c h w -> (b t) c h w')
                hidden_states = attn(hidden_states, temb=temb)
            ...
            hidden_states = resnets(hidden_states,
                                    temb,
                                    is_init_image=is_init_image,
                                    temporal_chunk=temporal_chunk)
...
if __name__ == "__main__":
    ...
    # down_encoder_block = DownEncoderBlock(in_channels=32,
    #                                       out_channels=32,
    #                                       )
    
    # x = torch.randn(2, 32, 8, 512, 512)
    # output = down_encoder_block(x)
    # print(output.shape)
    # -----------------------------------------------------------
    # down_encoder_block_causal_3D = DownEncoderBlockCausal3D(in_channels=32,
    #                                       out_channels=32,
    #                                       )
    
    # x = torch.randn(2, 32, 8, 512, 512)
    # output = down_encoder_block_causal_3D(x)
    # print(output.shape)
    # ----------------------------------------------------------------
    # _get_down_block = get_down_block(down_block_type="DownEncoderBlock2D",
    #                              num_layers=1,
    #                              in_channels=32,
    #                              out_channels=32,
    #                              temb_channels=None,
    #                              add_spatial_downsample=True,
    ...
    # x = torch.randn(2, 32, 8, 512, 512)
    # output = _get_down_block(x)
    # print(output.shape)
    # -------------------------------------------------------------------------
    _get_up_block = get_up_block(up_block_type="UpDecoderBlock2D",
                                 num_layers=1,
                                 in_channels=32,
                                 out_channels=32,
                                 temb_channels=None,
                                 add_spatial_upsample=True,
