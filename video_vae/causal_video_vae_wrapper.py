...
class CausalVideoVAEWrapper(nn.Module):
    ...
    def load_checkpoint(self,
        ...
        vae_ckpt_load_result = self.vae.load_state_dict(state_dict=vae_checkpoint,
                                                        strict=False)
        ...
        if self.add_discriminator:
            ...
            print(f"Load discriminator checkpoint from {checkpoint_path}, load result: {disc_ckpt_load_result}")
    ...
    def forward(self, 
        ...
        if is_context_parallel_intialized():
            ...
            torch.distributed.broadcast(tensor=x,
                                        src=global_src_rank,
                                        group=get_context_parallel_group)
            batch_x = conv_scatter_to_context_parallel_region(input_=x,
                                                              dim=2,
                                                              kernel_size=1)
    ...
    def numpy_to_pil(images):
        ...
        """
        Convert a numpy or batch of images to a PIL image.
        """
        ...
        if images.shape[-1] == 1:
            # spatial case for grayscale (single channel) images 
    ...
    def reconstruct(self,
        ...
        x = self.encode(
            ...
            is_init_image=is_init_image,
            temporal_chunk=temporal_chunk,
            window_size=encode_window_size,
            tile_sample_min_size=tile_sample_min_size
        )
        encode_latent = x 
        ...
        if return_latent:
            return output_image, encode_latent
    ...
    def decode_latent(self,
        ...
        # convert to PIL images 
        output_image = rearrange(output_image, 
                                 "B C T H W -> (B T) C H W")
        output_image = output_image.cpu().permute(0, 2, 3, 1).numpy()
        output_image = self.numpy_to_pil(output_image)
