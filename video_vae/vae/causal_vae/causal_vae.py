...
from middleware.gpu_processes import is_context_parallel_initialized
...
class CausalVAE(ModelMixin, ConfigMixin):
    ...
    def encode(
        ...
        if self.use_tiling and \
            ...
            assert NotImplementedError("please work in progress...")
    ...
    def forward(self,
        ...
        if is_context_parallel_initialized():
            ...
            if freeze_encoder:
                assert NotImplementedError("make sure freeze_encoder is not support.")

            else:
                h = self.encoder()

        else:
            assert NotImplementedError("without Gpu processing... is not working...")
