...
class CausalVaeEncoder(nn.Module):
    ...
    r"""
    The `Encoder` layer of a variational autoencoder that encodes its input into a latest representation.

    Args:
        in_channels (`int`, *optional*, default to 3):
            The number of input channels.
        out_channels (`int`, *optional*, default to 3):
            The number of output channels.
        down_block_types (`Tuple[str, ...]`, *optional*, default to `("DownEncoderBlock2D",)`):
            The types of down blocks to use. See `~diffusers.modules.models.unet_2d_blocks.get_down_block` for available options.

        block_out_channels (`Tuple[int, ...]`, *optional*, default to `(64,)`):
            The number of output channels for each block.
        layer_per_block (`int`, *optional*, default to 2):
            The number of layers per block
        norm_num_groups (`int`, *optional*, default to 32):
            The number of groups for normalization.

        act_fn (`str`, *optional*, defaults to `"silut"`):
            The activation function to use. See `~diffusers.models.activations.get_activation` for available options.
        double_z (`bool`, *optional*, default to `True`):
    ...
    def forward(self,
        ...
        if self.training and self.gradient_checkpointing:
            ...
            if is_torch_version(">=", "1.11.0"):
                ...
                # down block 
                for down_block in self.down_blocks:
                    sample = torch.utils.checkpoint.checkpoint(create_custom_forward(down_block),
                                                               sample,
                                                               is_init_image,
                                                               temporal_chunk)
                ...
                # middle block 
                sample = torch.utils.checkpoint.checkpoint(create_custom_forward(self.mid_block),
                                                           sample, 
                                                           is_init_image,
                                                           temporal_chunk)
...
class CausalVaeDecoder(nn.Module):
    ...
    Args:
        ...
        out_channels (`int`, *optional*, default to 3):
            The number of output channels.
        up_block_types (`Tuple[str, ...]`, *optional*, default to `("UpDecoderBlock2D")`):
            The types of up blocks to use. See `~diffusers.models.unet_2d_blocks.get_up_block` for available options.

        block_out_channels (`Tuple[int, ...]`, *optional*, defualt to `(64,)`):
            The number of output channels for each block.
        layers_per_block (`int`, *optional*, default to 2):
            The number of layers per block 
        norm_num_groups (`int`, *optional*, default to 32):
            The number of groups for normalization 

        act_fn (`str`, *optional*, default to `"silu"`):
            The activation function to use. See `~diffusers.models.activations.get_activation` for available options.
        norm_type (`str`, *optional*, default to `"group"`):
            The normalization type to use. Can be either `"groups"` or `"spatial"`.
    ...
    def __init__(
        ...
        super().__init__()
        self.layers_par_block = layers_per_block
        ...
        for i, up_block_type in enumerate(up_block_types):
            ...
            up_block = get_up_block(
                ...
                num_layers=self.layers_par_block[i],
