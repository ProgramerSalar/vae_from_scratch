...
class AdaptiveLossWeight:
    ...
    def __init__(
        ...
        self.bucket_ranges = torch.linspace(timestep_range[0], 
                                            timestep_range[1],
                                            buckets-1)
...
class LPIPSWithDiscriminator(nn.Module):
    ...
    def __init__(
            ...
            lpips_ckpt="flow/PATH/pyramid-flow-miniflux/lpips/vgg_lpips.pth",
            # --- Discriminator loss ---
            disc_num_layers=4,
            disc_in_channels=3,
            disc_factor=1.0,
        ...
        super().__init__()
        assert disc_loss in ["hinge", "vanilla"], "make sure disc loss have list ['hinge', 'vanilla']"
    ...
    def calculate_adaptive_weight(self,
        ...
        if last_layer is not None:
            nll_grads = torch.autograd.grad(outputs=nll_loss,
                                           inputs=last_layer,
                                           retain_graph=True)[0]
            g_grads = torch.autograd.grad(outputs=g_loss,
                                            inputs=last_layer,
                                            retain_graph=True)[0]
    ...
    def forward(self,
        ...
        inputs = rearrange(reconstructions, 'b c t h w -> (b t) c h w').contiguous()

        if optimizer_idx == 0:
            
            rec_loss = torch.mean(input= F.mse_loss(input=inputs,
            ...
            log = {
                "{split}/total_loss": loss.clone().mean(),
                "{split}/logvar": self.logvar.detach(),
                "{split}/kl_loss": kl_loss.detach().mean(),
                "{split}/nll_loss": nll_loss.detach().mean(),
                "{split}/rec_loss": rec_loss.detach().mean(),
                "{split}/perceptual_loss": p_loss.detach().mean(),
                "{split}/d_weight": d_weight.detach(),
                "{split}/disc_weight": torch.tensor(disc_factor),
                "{split}/g_loss": g_loss.detach().mean()
            }
        ...
        if optimizer_idx == 1:
            if self.using_3d_discriminator:
                ...
                print("make sure `using_3d_discriminator=True`")
