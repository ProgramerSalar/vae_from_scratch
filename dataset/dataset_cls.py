...
class ImageTextDataset(Dataset):
    ...
    """ 
        ...
        params:
            annot_file: The annotation file list 
            add_normalize: whether to normalize the input pixel to [-1, 1], default: True
    ...
    def get_closest_size(self, x):
        ...
        if self.p_random_ratio > 0 and \
            np.random.rand() < self.p_random_ratio:
    ...
    def __getitem__(self, index):
        ...
        image_annos = self.image_annos[index]

        try:

            img = Image.open(image_annos['image']).convert('RGB')
            text = image_annos['text']

            assert isinstance(text, str), "Text should be str"

            size = self.get_closest_size(img)
            resize_size = self.get_resize_size(orig_size=(img.width, img.height),
                                               tgt_size=size)
...
class LengthGroupVideoTextDataset(Dataset):

    """ 
        Usage:
            The dataset class for video-text pairs, used for video generation training 
            Its groups the video with the same frames together 
            Not only supporting fixed resolution during training

        params:
            anno_file: The annotation file list 
            max_frames: The maximum temporal length (This is the vae temporal length) 16 => (16 - 1) * 8 + 1 = 121 frames 
...
class VideoFrameProcessor:
    ...
    def __call__(self, 
        ...
        try:
            ...
            start_indexs = random.choice(start_indexs)

            filtered_frames = frames[start_indexs : start_indexs+self.num_frames]
...
class VideoDataset(Dataset):
    ...
    def __init__(self,
        ...
        print(f"The training video clip frame is {max_frames}")
...
if __name__ == "__main__":
    ...
    image_text_dataset = ImageTextDataset(anno_file='annotation/video_text.jsonl',
                                          )
    # print(image_text_dataset)

    # video_dataset = VideoDataset(anno_file='annotation/video_text.jsonl')
    # print(video_dataset)
