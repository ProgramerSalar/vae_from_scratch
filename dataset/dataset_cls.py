...
class ImageTextDataset(Dataset):
    ...
    It's support multi-aspect ratio training.

    params:
        anno_file: The annotation file list 
        add_normalization: whether to normalize the input image pixel to [-1, 1], default: True
        ratios: The aspect ratios during training, format: width / height 
        sizes: The resolution of training images, format: (width, height)
    """
    ...
    def get_closest_size(self, x):
        if self.p_random_ratio > 0 and \
            np.random.seed() < self.p_random_ratio:
...
class LengthGroupedVideoTextDataset(Dataset):
    ...
    It's groups the video with the same frames together 
    Now only supporting fixed resolution during training.

    Args:
        anno_file: 
        max_frames: the maximum temporal length (This is the vae letent temporal length) 16 => (16 - 1) * 8 + 1 = 121 frames 
    ...
    def __init__(self,
        ...
        assert load_vae_latent, "make sure `load_vae_latent=True`"
        assert load_text_fea, "make sure `load_text_fea=True`"
    ...
    def __getitem__(self, index):
        try:
            ...
            if self.load_text_fea:
                ...
                text_fea = torch.load(text_fea_path, 
                                      map_location='cpu')
...
class VideoFrameProcessor:
    ...
    def __call__(self,
        ...
        try:
            ...
            filtered_frames = frames[start_indexs : start_index + self.num_frames]
...
class ImageDataset(Dataset):
    ...
    def __init__(self,
        ...
        print(f"totaly Remained {len(image_paths)} images")
    ...
    def __getitem__(self, 
        ...
        try:
            ...
            print(f"what is the shape of filtered frame: {filtered_frames.shape}")
            filtered_frames = filtered_frames.permute(1, 0, 2, 3)
...
if __name__ == "__main__":
    # image_text_dataset = ImageTextDataset(anno_file="/home/manish/Desktop/stable-diffusion/flow/annotation/image_text.jsonl")
    # print(image_text_dataset)

    # length_grouped_video_text_dataset = LengthGroupedVideoTextDataset(anno_file='/home/manish/Desktop/stable-diffusion/flow/annotation/video_text.jsonl')
    # print(length_grouped_video_text_dataset)

    # video_frame_processor = VideoFrameProcessor()
    # video_frame_processor.__call__(video_path="/home/manish/Desktop/stable-diffusion/flow/webvid10m/train/010451_010500/23388121.mp4")
    # print(video_frame_processor)


    # video_dataset = VideoDataset(anno_file="/home/manish/Desktop/stable-diffusion/flow/annotation/video_text.jsonl")
    # print(video_dataset)

    image_dataset = ImageDataset(anno_file="/home/manish/Desktop/stable-diffusion/flow/annotation/image_text.jsonl")
    print(image_dataset)

    image_datloader = DataLoader(image_dataset,
                                 batch_size=1,
                                 shuffle=True)
    
    for i in image_datloader:
        print(i['video'])