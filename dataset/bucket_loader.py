...
class Bucketeer:
    ...
    def __next__(self):
            ...
            for dct in elements:
                try:
                    ...
                    self.buckets[size].append({
                        **{'video': img},
                        **{k:dct[k] for k in dct if k != 'video'}
                    })
        ...
        out = {
            k: [batch[i][k] for i in range(len(batch))]  for k in batch[0]
        }

        return {
            k: torch.stack(o, dim=0) if isinstance(o[0], torch.Tensor) else o for k, o in out.items()
        }
...
class TemporalLengthBucketeer:
    ...
    def __next__(self):
        ...
        if 'prompt_embed' in out:
            ...
            # Loading the pre-extracted textual features 
