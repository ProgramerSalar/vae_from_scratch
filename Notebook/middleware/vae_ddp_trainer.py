...
def train_one_epoch(
    ...
    print(f"start training epoch {epoch}, {iters_per_epoch} iteration per inner epoch")
    ...
    for step in metric_logger.log_every(iterable=range(iters_per_epoch), 
        ...
        with torch.amp.autocast(device_type="cuda",
            ...
            if rec_loss is not None:
                ...
                loss_scale_value = loss_scaler.state_dict()["scale"] if "scale" in loss_scaler.state_dict() else 1 
            ...
            if gan_loss is not None:
                ...
                if not math.isfinite(gan_loss_value):
                    print(f"The gan discriminator Loss is {gan_loss_value}, stopping training", force=True)
                    sys.exit(1)
                ...
                disc_loss_scale_value = loss_scaler_disc.state_dict()["scale"] if "scale" in loss_scaler_disc.state_dict() else 1
            ...
            new_log_loss = {
                k.split('/')[-1]:v
                for k, v in log_loss.items() if k not in ['total_loss']
            }
        ...
        # the new dict contains only the name of each metric as the key and it's global average value 
                            ## <-- Before --> ##
        # {
        #   'loss': <SmoothedValue object with a global_avg of 0.123>,
        #   'accuracy': <SmoothedValue object with a global_avg of 0.95>
        # }
                            ## <-- After --> ##
        # {
        #   'loss': 0.123,
