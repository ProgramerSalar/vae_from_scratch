...
class CausalVideoVAE(ModelMixin, ConfigMixin):
    ...
    def chunk_encode(self, x: torch.FloatTensor, window_size=16):
        ...
        # print(f'what frame to get {num_frames} and what downsample_scale: {self.downsample_scale}')
...
if __name__ == "__main__":
    ...
    # causal_video_vae = CausalVideoVAE().to(device)
    ...
    # print(f"what is the shape latent_z: {latent_z.shape} and dtype: {latent_z.dtype}") # torch.Size([2, 4, 2, 32, 32])

    # # Decode the latent representation back to video 
    # with torch.no_grad(), torch.amp.autocast("cuda", dtype=torch.float32):
    #     decoding_output = causal_video_vae.decode(z=latent_z,
    #                                               is_init_image=True,
    #                                               temporal_chunk=False,
    #                                               window_size=2)
    #     reconstructed_video = decoding_output.sample 
